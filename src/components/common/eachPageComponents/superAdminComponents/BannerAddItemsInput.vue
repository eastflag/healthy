    ...
    <v-container fluid class="boardLayout">
        <table>
            ...
            <tr style="border-top: 1px solid #E5E5E5">
                ...
                <th>전시타입</th>
                <td>
                    <div v-for="(item, index) in radioBoxUpType" :key="index" class="spot">
                        <input class="radiobx" type="radio" :id="'n'+item.id" name="radioN"><label :for="'n'+item.id"><em>{{ item.title }}</em></label>
                    </div>
                </td>
            </tr>
            <tr>
                <th>타입선택</th>
                <td>
                    <div v-for="(item, index) in radioBoxType" :key="index" class="spot">
                        <input class="radiobx" type="radio" :id="'t'+item.id" name="radioT"><label :for="'t'+item.id"><em>{{ item.title }}</em></label>
                    </div>
                </td>
                <th>노출여부</th>
                <td>
                    <div v-for="(item, index) in RadioView" :key="index" class="spot">
                        <input class="radiobx" type="radio" :id="'no'+item.id" name="radioY"><label :for="'no'+item.id"><em>{{ item.title }}</em></label>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <div class="selectbx d-flex">
                        <input type="text" id="cont" name="info1" placeholder="웹접근성을 위해 팝업내용에 대해 간략하게 서술해 주세요.">
                    </div>
                </td>
            </tr>
        </table>
